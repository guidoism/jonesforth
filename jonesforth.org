I can do this: LATEX_CLASS_OPTIONS: [letter,11pt,twoside,twocolumn]
...
Let's have a look at how =QUADRUPLE= and =DOUBLE= really look in Forth:
        ...
        3. jumps to the indirect =%eax=          (jumps to the address in the codeword of =+=, ie. the assembly code to implement =+=)
...
emit_scratch:
	...
	#+end_src
        ...
        ASCII =NUL= character as in C), and so Forth strings can contain any character including =NUL=s
...
* Compiling
	...
	#+end_example
        ...
        The issue is what happens next.  Obviously what we /don't/ want to happen is that we
        ...
        =IMMEDIATE= then the interpreter runs it immediately /even if it's in compile mode/.
	...
	#+end_src
        ...
        memory pointeid to by =HERE=, and adds 4 to =HERE=.  So the action of =,= (=COMMA=) is:
